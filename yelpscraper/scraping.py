...
def parse(url):
    ...
    if raw_json:
        ...
        #print(json_loaded['searchPageProps']['searchResultsProps']['searchResults'])
...
print("description: (ex:Therapist)")
search_query = input()
print("place: (ex:Los angeles)")
place= input()
yelp_url = "https://www.yelp.com/search?find_desc=" + search_query+"&find_loc="+place
print(yelp_url)
print ("Retrieving :", yelp_url)
scraped_data = parse(yelp_url)
with open("scraped_yelp_results2_for_%s.csv" % (place), "wb") as fp:
